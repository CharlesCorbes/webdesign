...
function ai(p){
	x = Math.floor(Math.random()*3)+1 //creates random number range (0-3)
	if (x==1){
...
function score (ai,p){
	...
	bestof = 10
	if (p==bestof){
		document.getElementById('over').innerHTML = "<div class='ino'><h1 class='win'>Victory! Your are great!</h1><input type='button' value='Reset' onClick='window.location.reload()'></div>"
		document.getElementById('over1').innerHTML = "<h1>You Won! Nice game!!</h1>"
	}else if(ai==bestof){
